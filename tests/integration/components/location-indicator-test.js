...
test("should reveal current location", function(assert) {
  ...
  assert.equal(this.$().text().trim(), "You are currently located in New York, USA");
});

test('should change displayed location when current location changes', function (assert) {
  this.render(hbs`{{location-indicator}}`);
  assert.equal(this.$().text().trim(), 'You are currently located in New York, USA', 'origin location should display');
  ...
  assert.equal(this.$().text().trim(), 'You are currently located in Beijing, China', 'location display should change');
});
